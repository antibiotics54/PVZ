...
#define danger1 "/Users/valeriaalfaro/Library/Developer/Xcode/DerivedData/PlantsZombies-fiokuwxnbevesecggirpjmijaqec/Build/Products/assets/png/danger2.png"
#define comiendo "/Users/valeriaalfaro/Library/Developer/Xcode/DerivedData/PlantsZombies-fiokuwxnbevesecggirpjmijaqec/Build/Products/assets/png/comiendo1.png"
...
//
void apply_pea_hitting_zombie(Objetos& elements, int p_ind, int z_ind, int &xp) {
...
void move_zombies(vector<Zombie>& zombies, int &xp) {
...
void apply_zombie_byte_on_peashooter(Objetos& obj, int z_ind, int p_ind, vector<vector<Block> > sc, int &xp) {
...
void handle_zombie_peashooter_encounter(Objetos& obj, vector<vector<Block> > sc, int &xp) {
    for (int i = 0; i < obj.peashooter.size(); i++)
        for (int j = 0; j < obj.zombies.size(); j++)
            apply_zombie_byte_on_peashooter(obj, j, i, sc, xp);
}

void apply_zombie_byte_on_walnut(Objetos& obj, int z_ind, int w_ind, vector<vector<Block> > sc, int &xp) {
...
void handle_zombie_walnut_encounter(Objetos& obj, vector<vector<Block> > sc, int &xp) {
    for (int i = 0; i < obj.walnut.size(); i++)
        for (int j = 0; j < obj.zombies.size(); j++)
            apply_zombie_byte_on_walnut(obj, j, i, sc, xp);
}
...
//funcion para imprimir los sunflowers
...
void MostrarZombies(SDL_Renderer* ren, vector<Zombie>& zombies, Objetos& obj, vector<vector< Block> >& sc, int &xp) {
    Zombie z;
    ...
    for (int i = 0; i < zombies.size(); i++) {
        ...
        if (zombies[i].health==10 && !ZombieChocadoConPlantas(obj, z, sc)) {
...
void crearPlantas(Objetos& obj, Icon& icon, int& sol_total, int& x, int& y, vector<vector< Block> >& sc) {
    ...
    if ((icon.is_peashooter_chosen) && (sol_total >= 100)) {
        ...
        icon.is_sunflower_chosen = false;
        sol_total -= 50;
...
//funcion para mostrar txt
void XP(SDL_Renderer* sprite, TTF_Font* font, int &xp){
    SDL_Color color = {0,0,0};
    string texto = to_string(xp);
    SDL_Surface* textSurface = TTF_RenderText_Solid(font, texto.c_str(), color);
    SDL_Texture* muestra = SDL_CreateTextureFromSurface(sprite, textSurface);
    SDL_Rect textRect;
    textRect.x = 1000;
...
int main(int argc, char* argv[]) {
    ...
    moveSunflower.x = moveSunflower.y =0;
    moveSunflower.w =29;
    moveSunflower.h = 33;
    moveWalnut.h =32;
    moveWalnut.w = 27;
    ...
    while (run) {
        ...
        if(player.xp>=1600){//ganado
            ...
            SDL_RenderCopy(renderer, experience , NULL, &xpPos);
            ...
            handle_zombie_peashooter_encounter(Object, sc, player.xp);
            handle_zombie_walnut_encounter(Object, sc, player.xp);
            handle_zombie_sunflower_encounter(Object, sc);
            
            move_zombies(Object.zombies, player.xp);
            MostrarZombies(renderer, Object.zombies,Object, sc, player.xp);
            ...
            for (int A = 0; A < Object.zombies.size(); A++) {
                for (int L = 0; L < Object.peas.size(); L++) {
                    if (IsZombieInPeaRow(Object.peas[L], Object.zombies[A])) {
                        apply_pea_hitting_zombie(Object, L, A, player.xp);
                    }
